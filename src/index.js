...
const { createWhatsAppClient, findGroupByName, listGroups } = require('./whatsapp/client');
...
async function main() {
  ...
  client.on('ready', async () => {
    ...
    if (config.photosGroupName) {
      photosGroup = await findGroupByName(client, config.photosGroupName);
      if (!photosGroup) log.warn(`קבוצת התמונות "${config.photosGroupName}" לא נמצאה - בקשות תמונות יישלחו לקבוצת הליקוט.`);
    }
