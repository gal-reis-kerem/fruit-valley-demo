...
function statsFor(orders) {
  ...
  return {
    ...
    documentedPct: n ? Math.round((orders.filter((o) => o.status === 'documented').length / n) * 100) : null,
  };
}

function fullStats(companies) {
  const db = store.loadDB();
  const real = db.orders.filter((o) => o.status !== 'archived');
  return {
